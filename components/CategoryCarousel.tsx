...
const CategoryCarousel: React.FC<CategoryCarouselProps> = ({
  ...
  const activeSubs = subcategories.filter(s => s.categoriaId === activeCategory);
            ...
            {categories.map((cat) => (
              ...
              >
                <div className={`relative aspect-square rounded-full overflow-hidden border-2 mb-4 transition-all duration-500 ${activeCategory === cat.id ? 'border-green-500 shadow-[0_0_30px_rgba(34,197,94,0.3)]' : 'border-zinc-800'}`}>
                  <img 
                    src={cat.midia || 'https://placehold.co/400x400/18181b/fafafa?text=PV'} 
                    alt={cat.nome}
          ...
          <div className="mt-16 animate-in fade-in slide-in-from-top-4 duration-700">
            ...
            >
              ...
              {activeSubs.map(sub => (
                ...
                >
                  {sub.midia && <img src={sub.midia} className="w-6 h-6 rounded-md object-cover" alt="" />}
                  {sub.nome}
                </button>
              ))}
            </div>
