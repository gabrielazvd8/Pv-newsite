...
const App: React.FC = () => {
  ...
  const isAdminDomain = hostname === "admin.pvsports.com.br";
  
  const [view, setView] = useState<AppView>(isAdminDomain ? 'admin' : 'store');
  ...
  useEffect(() => {
    // Redirecionamento de segurança: se tentar acessar /admin no domínio principal
    if (!isAdminDomain && window.location.pathname.includes('/admin')) {
      window.location.href = 'https://admin.pvsports.com.br';
      return;
    }
      ...
      <div className="min-h-screen bg-black text-white selection:bg-green-500/30 flex flex-col overflow-x-hidden">
        {isAuthenticated ? (
          <AdminDashboard 
            ...
            onBack={() => {}} // No back to store in admin domain
            onUpdate={loadAppData}
          />
        ) : (
          <AdminLogin 
            onSuccess={() => setIsAuthenticated(true)} 
            onCancel={() => {}} // No cancel in admin domain
          />
        )}
      </div>
    );
